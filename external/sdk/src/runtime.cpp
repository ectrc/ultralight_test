...
auto runtime::load_gobjects() -> std::expected<uobject_array, sdk::find_error> {
  ...
  _ASSERT(object_array_.array_ && "Failed to initialize UObjectArray");

  if (this->object_array_.array_ == nullptr) {
    return std::unexpected{sdk::find_error::offset_not_found};
  }
